...
var send = function send(url, data, progress, success, err) {
...
/**
 * Returns the mimetype matching the given extension
...
var getFileMetaDataFromBase64 = function getFileMetaDataFromBase64(base64) {
	var mimeType = getMimeTypeFromDataURI(base64);
	var extension = getExtensionByMimeType(mimeType);
	var name = 'unknown.' + extension;
	return {
		name: name,
		type: mimeType,
		size: Math.floor(base64.length * .73) // base64 is approx 37% bigger than final image, so let's reverse it to get the approx image size
	};
};

var getFileMetaDataFromUrl = function getFileMetaDataFromUrl(url) {
	var name = getFileName(url);
	var mimeType = getFileMimeType(name.split('.').pop());
	return {
		name: name,
		type: mimeType,
		size: null
...
var getFileMetaDataFromFile = function getFileMetaDataFromFile(file) {
	return {
		name: file.name,
		type: file.type,
		size: file.size
...
var getFileMetaData = function getFileMetaData(file) {

	if (typeof file === 'string') {

		// test if is base 64 data uri
		if (/^data:image/.test(file)) {
			return getFileMetaDataFromBase64(file);
		}

		// is url
		return getFileMetaDataFromUrl(file);
	}

	return getFileMetaDataFromFile(file);
};

var setFileMetaData = function setFileMetaData(fileData) {
	var extension = arguments.length <= 1 || arguments[1] === undefined ? '' : arguments[1];


	var data = clone(fileData);

	// correct extension if includes dot
	extension = extension.replace(/\./, '');

	// test if extension if of correct type, if not, force to png
	if (!CanvasExportExtensions.test(extension)) {
		extension = 'png';
	}

	// if no extension set new extension
	if (!ImageExtensionsRegex.test(data.name)) {
		data.name += '.' + extension;
	}
	// replace existing extension
	else {
			data.name = data.name.replace(/\.[a-z]+$/, '.' + extension);
		}

	// force mime type
	data.type = MimeTypes[extension];
...
var rotateCanvas = function rotateCanvas(original) {
...
var scaleRect = function scaleRect(rect, scalar) {
	return {
		x: rect.x * scalar,
		y: rect.y * scalar,
...
var flattenData = function flattenData(obj) {
	...
	if (inArray('output', props)) {
		data.output.image = toDataURL(obj.output.image, obj.input.type, jpegCompression);
	}
	...
	// so instead of testing the above situation we just always fix extension when handling png's
	var type = getMimeTypeFromDataURI(data.output.image || data.input.image);
	if (type === 'image/png') {
		data.input.name = getFileNameWithoutExtension(data.input.name) + '.png';
...
var Slim = function () {
   ...
   */
		...
		value: function _onInit() {
			...
			var done = function done() {
				...
				setTimeout(function () {
					_this12._options.didInit.apply(_this12, [_this12.data]);
				}, 0);
			};
   ...
   */
		...
		value: function _load(resource, callback) {
			...
			// test if is unknown file or if we should override the mime type
			if (file.type === 'unknown' || this._options.forceType) {
				file = setFileMetaData(file, this._options.forceType || 'png');
			}
			...
			getImageAsCanvas(resource, function (image, meta) {
				...
				var rewind = function rewind() {
					...
					// rewind state
					if (_this17._imageHopper) {
						_this17._imageHopper.enabled = true;
					}
				...
				if (!image) {
					...
					if (callback) {
						callback.apply(_this17, ['file-not-found']);
					}
				...
				if (!covers(image, _this17._options.minSize)) {

					rewind();

					_this17._onImageTooSmall();

					if (callback) {
						callback.apply(_this17, ['image-too-small']);
					}

					return;
				}

				var status = _this17._options.didLoad.apply(_this17, [file, image, meta]);
				if (status !== true) {

					rewind();

					if (status !== false) {
						_this17._throwError(status);
					}

					if (callback) {
						callback.apply(_this17, [status]);
					}
				...
				_this17._loadCanvas(image, function () {
					...
					var animation = {
						...
						complete: function complete() {
							...
							if (callback) {
								callback.apply(_this17, [null, _this17.data]);
							}
						}
		...
		value: function _loadCanvas(image, ready) {
			...
			this._applyTransforms(image, function (transformedImage) {
				...
				// store data (possibly)
				_this18._save(function () {}, willUpload);
		...
		value: function _applyTransforms(image, ready) {
			...
			transformCanvas(image, this._data.actions, function (transformedImage) {
				...
				_this19._onTransformCanvas(function (transformedData) {

					_this19._data = transformedData;

					_this19._options.didTransform.apply(_this19, [_this19.data]);
   ...
   */
				...
				intro.onload = function () {
					...
					setTimeout(function () {
						...
						_this21._showPreview(intro, function () {
							...
							// save the data
							_this21._save(function (err, data, res) {
			...
			this._applyTransforms(this._data.input.image, function (transformedImage) {
				...
				intro.onload = function () {
					...
					_this22._save(function (err, data, res) {
						...
						callback.apply(_this22, [_this22.data]);
					}, willUpload);
				};
			});
		}
		...
		value: function _save() {
			...
			if (!this._options.service && !(this._isInitialising && !this._isImageOnly())) {
				...
				this._options.willSave.apply(this, [data, function (data) {
					...
					_this23._options.didSave.apply(_this23, [data]);
				}]);
			}
			...
			if (this._options.service && allowUpload) {
				...
				this._options.willSave.apply(this, [data, function (data) {
					...
					_this23._upload(data, function (err, res) {

						// store response
						if (!err) {
							_this23._storeServerResponse(res);
						}

						// we did upload data
						_this23._options.didUpload.apply(_this23, [err, data, res]);
		...
		value: function _upload(data, callback) {
			...
			function (loaded, total) {
				...
				_this24._updateProgress(loaded / total);
			},
			...
			function (obj) {
				...
				setTimeout(function () {
					...
					statusNode.innerHTML = _this24._options.statusUploadSuccess;
			...
			function (status) {
				...
				if (status === 'file-too-big') {
					html = _this24._options.statusContentLength;
				} else {
					html = _this24._options.statusUnknownResponse;
				}
   ...
   */
		...
		value: function _hideButtons(callback) {
			...
			var animation = {
				...
				opacity: 0,
				allDone: function allDone() {
					_this25._btnGroup.style.display = 'none';
					if (callback) {
						callback();
					}
				}
		...
		value: function _doRemove() {
			...
			setTimeout(function () {
				...
				if (_this27._isBeingDestroyed) {
					return;
				}

				_this27._hideButtons(function () {
				...
				_this27._options.didRemove.apply(_this27, [data]);
		...
		value: function _doUpload(callback) {
			...
			this._hideButtons(function () {
				...
				// block upload button
				_this28._toggleButton('upload', false);

				_this28._save(function (err, data, res) {

					_this28._removeState('upload');
					_this28._stopProgress();

					if (callback) {
						callback.apply(_this28, [err, data, res]);
					}
		...
		value: function _doDownload() {
			...
			downloadCanvas(image, this._data.input.name, this._data.input.type);
		}
	}, {
		key: '_doDestroy',
		value: function _doDestroy() {
			...
			if (this._imageHopper) {
				HopperEvents.forEach(function (e) {
					_this29._imageHopper.element.removeEventListener(e, _this29);
				});
			...
			if (this._imageEditor) {
				ImageEditorEvents.forEach(function (e) {
					_this29._imageEditor.element.removeEventListener(e, _this29);
				});
			...
			nodeListToArray(this._btnGroup.children).forEach(function (btn) {
				btn.removeEventListener('click', _this29);
			});
			...
			attributes.forEach(function (attribute) {
				...
				if (matchesAttributeInList(attribute, _this29._originalElementAttributes)) {
					return;
				}

				// else remove
				_this29._originalElement.removeAttribute(attribute.name);
			});
			...
			this._originalElementAttributes.forEach(function (attribute) {
				...
				// add attribute
				_this29._originalElement.setAttribute(attribute.name, attribute.value);
			});
   ...
   */
		// properties
		get: function get() {
			return flattenData(this._data, this._options.post, this._options.jpegCompression);
		}
	}, {
		key: 'data',
		get: function get() {
		...
		value: function options() {
			...
			SlimButtons.concat(ImageEditor.Buttons).forEach(function (btn) {
...
(function () {
	...
	var toFloat = function toFloat(e, v) {
		if (!v) {
			return null;
		}
		return parseFloat(v);
	...
	// the will callbacks
	['Transform', 'Save', 'Remove'].forEach(function (cb) {
	...
	Slim.getOptionsFromAttributes = function (element) {
		...
		for (var prop in defaults) {
			...
			var _value = defaults[prop](element, getData(element, toAttribute(prop)));
			options[prop] = _value === null ? clone(Slim.options()[prop]) : _value;
		}
		return options;
	};
	...
	angular
		.module('slim', [])
		.directive('slim', function($timeout) {
			return {
				restrict: 'C',
				scope: callbacks,
				link: function(scope, element, attributes) {
					...
					$timeout(function () {
						...
						// get options
						var options = Slim.getOptionsFromAttributes(element[0]);
